 ...
 * 数据文件格式：
 * <p/>
 * 前DIR_ENTRY_COUNT项为索引，每项5个字节: status:1 offset:4
 * 索引后面是数据区, 数据区格式: length:text
 * <p/>
 * 文件名格式：
 * <p/>
 * queue_queuename.data.1
 * queue_queuename.data.2
 * ...
 * queue_queuename.data.n
 * <p/>
 * 文件编号单调递增，数据文件不重复使用.
 * <p/>
 * 使用完毕的数据文件在运行时会定时被删除，PersistQueueManagerImpl启动时也会对无用的文件进行清理。
 *
 * @author ranpengfei
 */
public class PersistQueueImpl implements PersistQueue {
    ...
    class MappedFile {
        ...
        int readPosition = -1; // 当前读位置
        int writePosition = -1; // 当前写位置
    ...
    public static boolean checkCompetedOnOpen(String filename) {
        ...
        try {
            ...
            for (; i < DIR_ENTRY_COUNT; ++i) {
                ...
                if (status == STATUS_AVAILABLE) { // 只有有用的数据项，才算文件可用
                    return false;
                }
            }
...
class UnMapTool {
    ...
    /**
     * 以下代码拷贝自toolkit下的queue实现:
     * svn://192.168.90.69/toolkit/trunk/toolkit-queue/src/main/java/com/snda/toolkit/queue/file/FilePageHeap.java
     * 提供一个unmap方法，否则map后的文件关闭后也无法删除, windows下确实有效.
     */
